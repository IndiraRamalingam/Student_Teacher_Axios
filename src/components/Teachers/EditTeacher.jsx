...
function EditTeacher() {
  ...
  const {
    ...
    const getTeacherDetails =async(params) =>{
      await axios.get(`https://64e32cc9bac46e480e7852ab.mockapi.io/Teachers/${params.id}`)
      .then(setTeachername(''),setContact(''),setSubject(''),setSalary(''))
      .then((response)=> ((setTeachername(response.data.teachername)),
                      (setContact(response.data.contact)),(setSalary(response.data.salary)),(setSubject(response.data.subject)) 
                      ));
        };  
        ...
        const UpdateTeacher =async(details)=>{
          await axios.put(`https://64e32cc9bac46e480e7852ab.mockapi.io/Teachers/${params.id}`, details)   
