...
function EditStudent() {
  ...
  const getStudentDetails =async(params) =>{
    await axios.get(`https://64e32cc9bac46e480e7852ab.mockapi.io/Students/${params.id}`) 
   .then(setName(''),setAddress(''),setAge(''),setContact(''),setSection(''),setTeachername(''))
   .then((response)=> ((setAge(response.data.age)), (setName(response.data.name)),(setAddress(response.data.address)),
                    (setContact(response.data.contact)),(setSection(response.data.section)),(setTeachername(response.data.teachername)) 
                    ));
     };  
  ...
  const UpdateStudent =async(details)=>{
    await axios.put(`https://64e32cc9bac46e480e7852ab.mockapi.io/Students/${params.id}`,details)   
